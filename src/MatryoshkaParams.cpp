...
MatryoshkaParams::MatryoshkaParams(std::shared_ptr<SparseMatrix> Ap, double gammap, size_t Kp, int minMeanSamples) :
 A(Ap), n(Ap->size1()), gamma(gammap), mu(std::vector<double>(Ap->size1()+1)), 
 sums(MatryoshkaParams::SymmetricMatrix(Ap->size1(), Ap->size2())), K(Kp), minMeanSamples(minMeanSamples) {
 computeSumMu_();
}
...
void MatryoshkaParams::computeSumMu_() {
...
void MatryoshkaParams::computeSumSigma_() {
	using namespace boost::accumulators;
	using namespace boost::range;
	// Vector to hold accumulators that will compute the mean
	// for domains of each size.
